import numpy as np
def get_standardized(X, *args, **kwargs):
    stan = Standardizer(**kwargs)
    X_stan = stan.fit_n_get(X)
    if not args:
        return X_stan
    else:
        return [X_stan] + [stan.transform(x) for x in args]
...
class Standardizer(object):
    ...
    def fit(self, X):
        ...
        if self.with_std:
            ...
            self.scales[abs(self.scales) < 0.0001] = 1.
